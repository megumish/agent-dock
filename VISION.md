...
1 と 2 は、どちらもユーザーが定めた制約に見えるが、意図的に分けている。分けるのは、定めた主体ではなく帰結の不可逆性である。受入基準やリスク上限の違反は事後に正せるが、不可逆な帰結 — 帰結を許容できるコストで元に戻せないこと（ユーザーが定めた境界の外への送信・秘密の開示・復元手段のない破壊・来歴の破壊）— だけは、事後の検証で学ぶこの体系で唯一、事後の修正が届かない。境界の内側で外部のモデルを使うことは、ここに触れない通常の運転である。
