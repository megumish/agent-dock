...
1. **安全・秘密・不可逆な帰結の回避**: スコアや効率で覆せない絶対制約。事後の検証で学ぶこの体系で、唯一事後の修正が届かない領域だからである。不可逆とは、帰結を許容できるコストで元に戻せないこと（ユーザーが定めた境界の外への送信・秘密の開示・復元手段のない破壊・来歴の破壊）。境界の内側で外部のモデルを使うことは、ここに触れない通常の運転である
