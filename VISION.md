...
AI モデルとツール、権限、実行基盤の組み合わせを**実行プロファイル**（execution profile）として登録し、実際の仕事で評価して役割ごとに使い分ける。
この文書では、登録した実行プロファイルを**乗組員**（crew member）[^crew]と呼ぶ。
...
- **評価**（evaluation）：実際の仕事から証拠を集め、乗組員の適性を表す評価結果を導く
- **振り分け**（routing）：評価結果に基づいて、指示を適切な乗組員に委ねる

振り分けにおける一回ごとの委任判断を**采配**（dispatch）と呼ぶ。
ドックでは、仕事の過程と帰結を記録する単位を**評価イベント**（evaluation event）と呼ぶ。
ドックが自律判断を止め、理由、選択肢、推奨案を示してユーザーへ判断を求める手続きを**エスカレーション**（escalation）と呼ぶ。
...
- **実行の実測**（execution measurement）：所要時間、実支出、帰結（受け入れ、手戻り、破棄）
- **決定**（decision）：委任先、委ねる範囲、エスカレーションといった采配上の判断、および遂行中の技術判断
- **フィードバック**（feedback）：即時の評価、および後日判明する長期の帰結
  タスクの境界より後に届く評価も、**遅延イベント**（late event）[^late-event]として同じ基盤に記録する。
...
評価イベントの集合に対象範囲、評価軸、集約方法を定めて導くビューを**射影**（projection）と呼ぶ。
**スコア**（score）は独立した実体ではなく、この射影の値である。
...
- **意思決定の確かさ**（decision calibration）[^calibration]：決定時に期待した帰結と、観測された帰結との整合度
  選ばなかった選択肢の帰結は観測できないため、観測した範囲を超える因果は断定しない。
- **作業速度**（turnaround time）[^turnaround]：指示を受けてから、受け入れ可能な成果を得るまでの実測時間
- **成果あたり実コスト**（actual cost per accepted outcome）：受け入れた成果一件に要した実支出
...
資格を持たない候補は、較正のための**観測状態**（shadow state）でだけ評価を出力できる。
...
この仕組みを**評価の代行**（delegated evaluation）[^delegated-evaluation]と呼び、次の制約を課す。

- **判断の優先**（user-judgment precedence）：代行評価はユーザー判断に常に劣後する別種のイベントであり、ユーザー判断が届けばそちらが現在の判定になる
- **常時監査**（continuous audit）：代行が評価した一部をユーザーも必ず評価し、一致率を継続して測る
- **領域の限定**（domain restriction）：代行を許すのは、ユーザー判断との一致実績がある領域だけとする
- **独立性**（independence）：審判役には被評価者と異なる実行プロファイルを使い、プロバイダ、モデル系列、プロンプト、ツール、参照できる証拠の共通性から偏りの共有リスクを評価する
...
受け入れ済みの実タスクを再演する評価を**リプレイ**（replay）と呼ぶ。

1. **実タスクの実測**（real-task measurement）：一次の証拠とする。
   対象との関連性、条件の比較可能性、標本の十分性を満たす実測は、ほかの源泉よりも重く扱う。
2. **自前リプレイ**（self-replay）：受け入れ済みの成果が確定している実タスクを再演した証拠とする。
   同じ実行プロファイルを過去の状態と比較してドリフトを検知する評価を**定点観測**（drift monitoring）[^drift-monitoring]と呼ぶ。
   リプレイは、新しい実行プロファイルの初期評価と較正に使うほか、定点観測にも使う。
   自前リプレイは、ユーザー自身の履歴を比較用の課題に変える。
3. **外部ベンチマーク**：「外部でこの主張が公開された」という来歴として記録する。
   性能の一次証拠とはせず、**外部事前情報**（external prior）として一次の評価イベントと区別したまま射影へ合成する。

リプレイは、新しい実行プロファイルの初期審査（initial review）と、すでに認めた役割適性の見直しの双方に使う。
後者の審査を**再審査**（requalification）[^requalification]と呼ぶ。
...
評判の変化や新バージョンの公開情報といった**外部シグナル**（external signal）は、スコアの証拠には使わず、再審査を起動するトリガーとしてだけ使う。
外部シグナルをトリガーに限定すれば、自前の証拠が蓄積する前に変化の兆候を得ても、未検証の評判はスコアへ混入しない。
価格改定は証拠ではなく実コスト計算の入力であるため、確認でき次第反映する。

### 役割ごとの資格

ドックは、采配役（dispatcher）、設計役（architect）[^architect]、実装役（implementer）、検証役（verifier）などの**役割**を定める。
...
各役割には、関連する射影への要求条件である**適性要件**（aptitude requirements）を定める。
実行プロファイルが特定の役割の適性要件を満たすと認めた状態を、その役割の**資格**（qualification）と呼ぶ。
...
采配は、現在の証拠に基づいて最も適した乗組員を選ぶ**活用**（exploitation）だけを目的としない。
証拠が少ない、または古い実行プロファイルに低リスクの実際の仕事を意図的に割り当てる**探索**（exploration）[^bandit]も担う。
...
[^crew]: crew member は、ドック（船渠）の比喩に合わせた語であり、集合は crew と書く。

[^late-event]: late event は、ストリーム処理で境界より後に届くイベントを指す慣用語に合わせる。

[^calibration]: 期待した帰結と観測された帰結の整合度を測るため、正答率（accuracy）ではなく較正（calibration）の語を充てる。

[^turnaround]: 指示から受け入れ可能な成果までの実測時間を指すため、speed ではなく所要時間の慣用語である turnaround time を充てる。

[^delegated-evaluation]: 采配の委任（delegation）と紛れないよう、単独の delegation とは書かず delegated evaluation の複合語で書き分ける。

[^drift-monitoring]: 直訳の fixed-point observation は数学の不動点を連想させるため、目的に即して drift monitoring と名付ける。

[^requalification]: 資格（qualification）の再判定であるため、re-review ではなく資格の語に接続した requalification を充てる。

[^architect]: 役割名としての慣用に合わせて architect を充てる（文書や設計行為の設計は design と書く）。

[^bandit]: exploitation と exploration の対は、バンディット問題の慣用に合わせる。
