...
AI モデルとツール、権限、実行基盤（execution platform）の組み合わせを**実行プロファイル**（execution profile）として登録し、実際の仕事で評価して役割ごとに使い分ける。
この文書では、登録した実行プロファイルを**乗組員**（crew member）と呼ぶ。
...
- **フィードバック**：即時の評価、および後日判明する長期の帰結
  タスクの境界より後に届く評価も、**遅延イベント**（late event）として同じ基盤に記録する。

イベントには、使用したモデル、プロバイダ、バージョン、ツール、権限、実行基盤を**構成**（configuration）として記録する。
...
射影は証拠の**鮮度**（freshness）を考慮し、古い証拠ほど重みを下げる。
鮮度に応じた重みづけによって、スコアから評価対象の現在の状態を推定する。
射影は値だけでなく、確からしさを示す**不確実性**（uncertainty）と、主張できる範囲を示す**適用範囲**（scope）を伴う。

代表的な評価軸は次のとおりである。

- **意思決定の確かさ**（decision calibration）：決定時に期待した帰結と、観測された帰結との整合度
  選ばなかった選択肢の帰結は観測できないため、観測した範囲を超える因果は断定しない。
- **作業速度**（turnaround time）：指示を受けてから、受け入れ可能な成果を得るまでの実測時間
...
一方、作業時間や実支出のような**観測事実**（observed fact）は別種の一次データであり、価値判断によって覆さない。
...
この仕組みを**評価の代行**（delegated evaluation）と呼び、次の制約を課す。
...
ドックは、采配役（dispatcher）、設計役（architect）、実装役（implementer）、検証役（verifier）などの**役割**（role）を定める。
...
ここでの**低リスク**（low-risk）とは、失敗の帰結が小さく、可逆であり、やり直しと成否判定に要するコストが低い仕事を指す。
采配は、現在の証拠に基づいて最も適した乗組員を選ぶ**活用**（exploitation）だけを目的としない。
証拠が少ない、または古い実行プロファイルに低リスクの実際の仕事を意図的に割り当てる**探索**（exploration）も担う。
...
ここで、許容できるコストで元に戻せない帰結を**不可逆な帰結**（irreversible consequence）と呼ぶ。
