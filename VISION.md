...
采配の責務は「活用」（今の最良の配置）だけではない。証拠が薄い・古い実行プロファイルに低リスクの実仕事を意図的に配る「探索」、そして鮮度切れ・ドリフト検知・外部シグナルを受けて起動する「リプレイ（再審査）」も担う。使われない乗組員の評価が止まる袋小路は、ここで断ち切る。
...
1. **安全・秘密・不可逆な帰結の回避**: スコアや効率で覆せない絶対制約
2. **ユーザーが定めた規範**: 受入基準とリスク上限
3. **証拠の十分性**: 確信が持てなければ委譲しない
4. **経済性**: ユーザーの手間を含む総コストと速度
5. **探索の価値**: 確保された予算の内でのみ

第1層と第2層は、どちらもユーザーが定めた制約に見えるが、意図的に別の層にしている。分けるのは、定めた主体ではなく帰結の不可逆性である。受入基準やリスク上限の違反は事後に正せるが、不可逆な帰結 — 帰結を許容できるコストで元に戻せないこと（ユーザーが定めた境界の外への送信・秘密の開示・復元手段のない破壊・来歴の破壊）— だけは、事後の検証で学ぶこの体系で唯一、事後の修正が届かない。境界の内側で外部のモデルを使うことは、ここに触れない通常の運転である。
...
- **ベンダー固定しない**: 特定ベンダーへの最適化やロックインをしない。乗組員である実行プロファイルは常に交換可能であり、新モデルを組み込んだプロファイルの追加と、既存モデルを載せたプロファイルの退場が常態である
