...
**出口**:
- 検証証拠: 縦切り1件の記録と来歴の追跡／最小ガードの fail-closed 動作／**安全 negative tests**（bypass 不能・未許可 egress の拒否・秘密の非送信/非記録・単発許可の scope/expiry/再利用拒否・保持削除の契約動作）／run 非依存イベント（方針変更）が封筒で表現できる
- stop/rollback: スキーマの作り直しは disposable なテスト名前空間のみで許す（正典記録を消さない）
- 確定 ADR: D-02（封筒・v0 ストア）・D-13（identity）・D-14・D-15・D-16・D-19（原則）・D-21
- 意図的未決: 最終ストア（D-02 の再評価トリガつき）・射影式・減衰値・采配アルゴリズム・taxonomy
...
- 確定 ADR: D-18・D-24（minimal→full）・D-01・D-06・D-08（価格 v0）・D-20
- 意図的未決: taxonomy・射影・采配・リスク分類
...
- 意図的未決: 較正方式・減衰の数値・采配方式・最終ストア（再評価トリガ監視中）
...
- 意図的未決: 代行の教師信号化（5b まで封印）・資格の執行（5 まで封印）・routing 選定・探索予算
...
- 意図的未決: 探索予算の数値（探索開始前）・執行用 risk ADR（Phase 5 前）・資格執行
...
- 意図的未決: なし（構造上の Decision は確定済み。運用リズム等の非構造的事項のみ残る）
...
- 確定 ADR: D-12（最終形）
- 意図的未決: なし（理由: 本計画の構造的 Decision はここで全て確定。以後の変更は新しい D-xx として登録）
...
| 成功基準の継続監視 | ベースライン台帳 | Phase 5 | 初回改善後の品質・失敗率・総コスト・attention の維持 | 非劣性の破れで delegated mode を停止 |
