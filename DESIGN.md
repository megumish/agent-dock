...
最初の **v0.0.0** は、指示の入力、実行プロファイルの選択、ユーザーの明示承認、ヘッドレス実行、結果の表示までを実装する。
v0.0.0 の対応 OS は macOS に限り、ほかの OS での動作は保証しない。
評価イベントの記録、受け入れ判定、スコアカード、助言は後続のパッチ版で追加する。
したがって v0.0.0 の完成をもって、v0.0 の循環または VISION の成功基準を達成したとはみなさない。
