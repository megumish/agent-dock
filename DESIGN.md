...
この文書では、外部から取り込む仮説入力を**外部事前情報**（external prior）と呼ぶ。
...
複数の射影について、値、不確実性、適用範囲、鮮度を並べて表示するビューを**スコアカード**（scorecard）と呼ぶ。
...
**正典記録**（canonical record）とは、何が起き、誰が何を主張したかを確定する記録である。
イベントログは正典記録を保持する。
...
**証拠階級**（evidence class）は、命題への寄与を証拠の種類ごとに定める序列である。
...
**ユーザー規範**（user norms）は、証拠から導出せず、ユーザーが定める運用上の基準である。
...
**能力申告**（capability declaration）は、測定できる項目、停止できる操作、観測できない項目を示す。
**実行レシート**（execution receipt）は、実際に使った構成と、取得できる範囲の実サーバー情報およびバージョン情報を示す。
...
成果の残存、手戻り、採用などから得る**受動シグナル**（passive signal）は、ユーザーの価値判断を推測する観測または代理信号として記録する。
...
証拠が少ない実行プロファイルへ低リスクの仕事を割り当てるための上限を**探索予算**（exploration budget）と呼ぶ。
無作為化または期間切替比較のために追加で負う費用、リスク、ユーザーの手間の上限を**介入予算**（intervention budget）と呼ぶ。
...
- **適合性マトリクス**：ガードの介入、中止、実行レシート、結果不明の表現、保守性、ベンダー中立性を比較する。
  D-01 はこのマトリクスを参照する。
- **最小契約**（minimum contract）：最初の実環境での実行から VISION 制約を満たすために必要な意味項目を指す。
  ...
  - 実現機構は D-24 で決める。
- **完全な契約**（complete contract）：最小契約を保ったまま、実行基盤ごとの項目対応、中止、再試行、部分失敗、タイムアウトの正規化規則を確定した契約を指す。
...
プロジェクト固有の概念の英語表記は、VISION と DESIGN の初出定義への併記を正本とする。
実装で用いる識別子は、他の設計決定と同じく ADR で確定する。
ADR には、採用する案と理由に加えて、その決定に関わる用語と識別子の英語表記を記載する。
本文に併記しない一般的な語の英語は、識別子を必要とする最初の ADR で確定する。
英語の選定に判断が入った場合は、その理由を ADR に残す。
ADR が本文の併記と異なる表記を採用する場合は、同じ改定で本文の併記も更新する。
