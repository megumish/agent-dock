...
| 種別 | 拘束力 | 例 |
|---|---|---|
| **設計不変条件** | VISION から導かれる論理契約。実装がどうであれ満たす。変更には VISION 改定が先に必要 | ドックが起動・仲介する全実行がガードを通る／イベントは追記のみ |
...
### イベントログ
発生・主張の**正典記録**（追記のみ）。封筒は型付き: 共通必須項目は最小に保ち（候補: id・時刻・種別・source/actor・因果参照・schema_version）、実行プロファイル参照は実行関連のイベント型でのみ必須。本文の扱いは「最小記録・全文の既定保存をしない」が制約で、参照方式は選択肢。関連: D-02, D-14, D-19。
...
- VISION制約: 失敗も証拠。イベントは追記のみ。**exactly-once は約束しないが、発見不能な欠落は許さない**（検出と突き合わせを必須とする）。
