...
- v0.0 が完成するまでは、実装方式を柔軟に探索する。
  ...
  設定とイベントの `schema_version` には実装のバージョンを使い、版が変わった記録は破棄して貯め直す。
  テストは過去の構造を保存するためではなく、そのパッチ版で採択した仕様と不変条件を検証するために置く。
- 拡張の順序は事前に固定しない。
...
- v0.0 の記録は使い捨て前提とする。
  移行は保証せず、現在の実装バージョンと一致しない設定とイベントは捨てて貯め直す。

### テスト方針

v0.0 のテストは、採択中の仕様、不変条件、実際に発生した事故の再発防止に絞る。
