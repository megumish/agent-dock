...
0.1.0 の完成条件は前述の最小ループ全体であるが、実装と検証は alpha に分けて進める。
最初の **0.1.0-alpha.1** は、指示の入力、実行プロファイルの選択、ユーザーの明示承認、ヘッドレス実行、結果の表示までを実装する。
0.1.0-alpha.1 の対応 OS は macOS に限り、ほかの OS での動作は保証しない。
**0.1.0-alpha.2** は、JSONL 形式の評価イベントと受け入れ判定を追加する。
**0.1.0-alpha.3** は、評価イベントから実行プロファイル別のスコアカードを計算して表示する。
**0.1.0-alpha.4** は、実行前のタスク再編集、過去実行の判定追記、未実行タスクの警告を追加する。
助言は後続の alpha で追加するため、0.1.0-alpha.4 の完成をもって、0.1.0 の循環または VISION の成功基準を達成したとはみなさない。
