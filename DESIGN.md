...
- alpha の設定は製品版から独立した `api_version` で識別し、版をまたぐ互換性と移行を保証しない。
  非対応の設定は、既定の対話コマンドでユーザーが既定値 No の確認を許可した場合だけ退避して作り直す。
- alpha の JSONL イベントは一時的な保存形式であり、製品版から独立した `format_version` で識別する。
  版をまたぐ互換性と移行は保証せず、非対応のイベントは、既定の対話コマンドでユーザーが既定値 No の確認を許可した場合だけログ全体を退避して空にする。
...
来歴は手動マーク、CLI の hook、通知など、事実を取得した経路を表す。
...
- **観測異常**：対応する開始がない終了マークと、取得できなかった終了通知を、セッション単位の診断として記録する。
...
- **実行アダプタ**：承認された実行プロファイルで CLI をヘッドレス起動し、所要時間や実支出などの実測を正規化してイベントにする。
  実際に使った構成も記録する。
  CLI の機械可読出力を使い、応答本文の提示と、公式に提供される申告値の取得を両立する。
- **観測アダプタ**：ユーザーが直接起動した Claude Code と Codex CLI から、セッションと構成を受け取り、共通イベントへ正規化する。
  タスク境界と受け入れ判定は、ユーザーの明示マークからだけ記録する。
- **記録**：評価イベントをローカルに追記で保持する。
- **射影**：記録からスコアカードを計算する。
- **助言**：スコアカードを根拠に候補を提示し、証拠が不足しているか候補を一意に選べなければ判断保留を返す。
...
CLI が公式の機械可読出力で申告した推定コストとトークン利用量を**申告値**（reported value）と呼ぶ。
トークン利用量は実支出を構成する入力に当たる使用量の開示であり、CLI 申告の推定コストは請求と無関係の名目値である。
申告値は観測事実の実測ではなく、教師信号でも外部事前情報でもない。
外部事前情報が実測の蓄積に応じた重みで射影へ合成される対象である（0.1.0 では合成自体を実現しない）のに対し、申告値は射影へ合成せず、評価軸、順位づけ、候補選定に使わない。
申告値は監査用の付帯記録としてイベントに残し、スコアカードでは実支出と明確に区別して開示する。
モデル単位の参考集計、参考射影とは別の概念である。
スコアカードの軸は受入率、所要時間、実支出の三軸のまま変えず、申告値を軸にしない。
...
- 候補には、参照したスコアカードの値を根拠として添える。
  根拠はスコアカードの三軸の値に限り、申告値を含めない。
- 指示と同じタグの実行実績が無い場合、または件数が少ない場合は判断保留を返す。
  件数の閾値は実装で調整する。
- 実績が足りていても、候補を一意に選べない場合は判断保留を返す。
