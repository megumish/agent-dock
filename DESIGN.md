...
- 問い: リスク分類（裁定の序列 1〜2 の機械化）と探索・介入予算の管理。
- VISION制約: 裁定の序列 1 は自律で越えない。探索は低リスク・可逆・予算内。リスク=帰結の大きさ・可逆性・判定コスト。
...
- VISION制約: 境界の外への送信・秘密の開示・不可逆な帰結は自律で行わない（裁定の序列 1）。ドックは実行系を所有しない（仲介する実行にのみ責任を持つ）。**保証できない capability は fail-closed**（その操作を許さない）。パターン検査のような補助防御を単独で境界執行にしない。
