...
| **shadow** | ユーザー（推薦は計算・記録されるが、提示も実行もされない） | 計測のみ | 通る | Phase 4a |
| **advisory** | ユーザー（推薦＋根拠＋abstain を提示し、毎回明示承認を経て実行） | 提示 | 通る | Phase 4b |
...
各項目のフィールド: **状態** / **問い** / **VISION制約**（確定）/ **選択肢と調査**（既存 OSS・標準の候補を含む。存在しなければ「なし＋理由」を明示）/ **証拠計画** / **依存** / **決定境界**（not before / no later than を両方明示。該当なしは「なし」）/ **ADRトリガ** / **可逆性** / **有力候補（非拘束）**。
...
**採用か自作かは開いた選択肢**: 各決定点には既存の OSS・既存ツールの採用と自作の両方を含め、どちらも既定としない。選定は ADR の場で、契約への適合と維持コストに基づいて判断する。
...
- 選択肢と調査: **封筒**（具体フィールドは ADR 対象。候補: 共通=id・時刻・種別・source/actor・因果参照・schema_version、実行関連型のみプロファイル参照必須）。参考標準: CloudEvents・OpenTelemetry trace/span・W3C Trace Context。**本文参照**: content-addressed（参照＋ハッシュ）/ パス参照 / ユーザー明示時のみ本文保持。**ストレージ**: SQLite（導入済・トリガで更新禁止）/ JSONL 追記＋派生インデックス / DuckDB（未導入）。完全性は D-24 と接続。
...
- 有力候補（非拘束）: 封筒6項目＋型別拡張、本文は参照＋ハッシュ、v0 ストアは SQLite か JSONL。
...
- 問い: タスク種別タグの初期体系と育て方。
- VISION制約: 第二軸を固定しない。
- 選択肢と調査: 手動シードの粗い種別（設計/実装/大量編集/調査/レビュー/運用）＋自由タグ。付与は采配時＋訂正可能（**ただし過去の cohort/holdout membership は書き換えない — D-16 の凍結契約**）。**既存標準: なし**（汎用の作業分類標準は個人のエージェント作業に適合しない。この判断も ADR で再確認）。
...
- 状態: open
- 問い: 再演の分離・実行・**採点（oracle）**と VCS アダプタ。
- VISION制約: リプレイは第二階級。定点観測に使う。予算内。裁定ガード経由・外部副作用なし（task-domain 副作用の禁止 — 定義はリプレイハーネス責務を参照）。
- 選択肢と調査: **VCS アダプタ**（このリポジトリは jj（colocated git）管理）: jj workspace / git worktree / 隔離コピー / コンテナ（Docker・Podman）。**採点（oracle）**: behavioral test / acceptance invariant / outcome oracle / ユーザーまたは資格ある独立 proxy の評価 — **受け入れ済み成果との diff は診断材料であり、採点 oracle にしない**。oracle を用意できない題材は hold（審査に使わない）。**answer leakage 防止**（受け入れ済み成果を審判・被験プロファイルに見せない）を契約化。非決定性: n 回サンプル。
...
- 選択肢と調査: サンプリング: 初期高率→逓減（領域別の **sample floor** を置く）。一致統計の既存 OSS: statsmodels / scikit-learn（κ係数）・krippendorff。**独立性は次元で定義**: provider / model family / 実行プロファイル / prompt・tools / evidence access（審判が被評価の証拠・成果に触れられるか）。停止閾値は**信頼区間つき**で定義。**監査欠落・不一致後の recovery 条件**（何を満たせば再開するか）も ADR 項目。継続 proxy 監査レーン（PLAN）と接続。
...
- 可逆性: 高。
- 有力候補（非拘束）: 逓減サンプリング＋次元定義された独立性。

### D-13 実行プロファイルの同一性（意味契約）

- 状態: open（identity は Phase 0 で ADR 必須）
- 問い: identity・バージョン・等価性・証拠の可搬性。
- VISION制約: 成績はプロファイルに帰属。構成変更は再審査事由。モデル名だけで資格を主張できない。モデルの同一性は名前では信じない（観測で疑う）。
- 選択肢と調査: identity: 構成要素の正規化ハッシュ＋可読名（参考: OCI イメージ設定・ロックファイル慣行）。**構成ハッシュだけでは「同じモデル名のままサービス側が変わる」を識別できない**ため、取得可能な範囲の actual receipt / server version を記録し、取得不能は unknown capability として明示（D-18）。**定点観測でドリフトを検知したら evidence portability を停止し再審査**（D-10 接続）。**識別子に秘密を含めない**。保持方針により識別子も削除・再発行可能とする。
...
- 選択肢と調査: 構造: 厳密な木は fan-out/fan-in・共有成果物・遅延 outcome を表せないため、DAG / many-to-many 参照も選択肢。参考標準: OpenTelemetry trace/span・W3C Trace Context・CloudEvents。遅延接続: 成果物参照経由 / 明示 task 参照。帰属: 規則確定まで「共同」のまま保持。
...
- 状態: open（Phase 0 で ADR 必須）
- 問い: 規範の**表現**と、**保管・認証（authority root）** — エージェントも編集できる環境で user-only 変更をどう保証するか — および恒久変更/単発許可の区別。
- VISION制約: 価値判断の ground truth はユーザー。境界の外への送信は自律で行わない。削除はユーザー起点。単発許可はエスカレーションで与えられ、資格を生まない。
- 選択肢と調査: **表現**: 宣言的設定ファイル（YAML・バージョン管理・変更もイベント化）/ 汎用ポリシー言語（OPA/Rego・Cedar）。**保管・認証**: agent-unwritable store（エージェントの書込み権を持たない場所）/ ホスト UI での out-of-band 確認 / **署名・nonce つき単発許可**（再利用不能）/ OS 権限による保護。scope・precedence・deny-by-default・失効/取り消し・**run 開始時の方針スナップショット**を含めて決める。**negative test**（D-21 と共有）: 不正変更の拒否・scope/expiry 超過の拒否・許可の再利用の拒否。上限値の設定はユーザーの領分（ドックが推奨を添えるかは非拘束の選択肢）。
- 証拠計画: Phase 0 縦切りで、境界執行（D-21）が方針スナップショットを読んで fail-closed に動くことと negative test。
- 依存: なし（他が依存する側）。
- 決定境界: not before: なし / no later than: Phase 0。
- ADRトリガ: Phase 0 開始。
- 可逆性: 表現形式は中・記録済み許可は不変。
- 有力候補（非拘束）: 宣言的設定ファイル＋run 開始時スナップショット＋署名つき単発許可。
...
- 選択肢と調査: **カタログ**: VISION の代表3軸を v0 の出発点とし、各軸の意味・入力イベント・計算対象を Phase 2 入口 ADR で定義。軸の追加・変更は版管理（D-19）に従い、D-04 の適性要件はこのカタログの軸を参照する。不確実性: 有効標本数 / ベータ区間 / ブートストラップ。較正の既存 OSS: scikit-learn（calibration）・MAPIE（conformal prediction）。バイアス: 探索データの重み付け・適用範囲の明示。**propensity が取れない場合は適用範囲を狭め、因果主張を禁止する。**
...
- 可逆性: 高。
- 有力候補（非拘束）: 最小形は代表3軸のカタログ＋有効標本数＋区間表示。
...
- VISION制約: 境界の外への送信・秘密の開示・不可逆な帰結は自律で行わない（裁定第1層）。ドックは実行系を所有しない（仲介する実行にのみ責任を持つ）。
- 選択肢と調査: 継続執行の方式: runtime sandbox / tool mediation（ツール呼び出しの仲介点で検査）/ egress policy（ネットワーク層の許可リスト）— いずれか、または併用。**保証できない capability は fail-closed**（その操作を許さない）。run 開始時の方針スナップショット。**negative test**: bypass 不能・未許可 egress の拒否・秘密の非送信/非記録・単発許可の scope/expiry/再利用拒否。秘密パターン検査（gitleaks / trufflehog ルール流用）は**補助防御**であり単独で境界執行にしない。
- 証拠計画: Phase 0 縦切り（dummy）で fail-closed 動作と negative test 群。real 実行の開始前に該当 CLI の継続執行方式を確認。
- 依存: D-15。
...
- 有力候補（非拘束）: 許可リスト（egress policy）＋tool mediation の併用＋方針スナップショット。
...
- 選択肢と調査: 再構成の実現: 決定イベントに参照を埋める（軽量）/ 定期スナップショット（重い・削除との両立に注意）。受入テスト: 過去の任意の決定を選び、監査面だけで上記を答えられるか（削除済みは unavailable 表示になるか）。**既存物: N/A** — 監査再構成は本システム固有の契約であり汎用 OSS は存在しない（event-sourcing の監査パターンを設計の参考にする）。
...
- 可逆性: 中。
- 有力候補（非拘束）: at-least-once＋レシート突き合わせ＋outbox 参考。
