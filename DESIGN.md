...
v0.0 の完成条件は前述の最小ループ全体であるが、実装と検証はパッチ版に分けて進める。
...
具体的な対応 CLI はリリース文書に記録する。

ヘッドレス実行と対話セッション経由の違いは、実行プロファイルを構成する実行基盤の違いとして扱う。
v0.0 の実行基盤はヘッドレス実行に限る。
対話セッションを介した実行は、タスク境界と実測のタスク単位の帰属が煩雑になるため v0.0 では仲介せず、実行基盤の追加として後日扱う。

ドックを介さずにエージェント CLI を直接実行した仕事も、将来は観測対象にする。
これはドックによる実行の仲介とは別の入口であり、v0.0 の後続パッチ版で扱う。
...
v0.0 では、識別子を UUIDv5 と正準バイト列から生成する。
同じ宣言内容を異なる表示名で重複登録することはできない。
設定に `test` CLI を記述することはできず、安全なテストプロファイルはシステムが常に先頭へ補う。

小さな構成変更をまたいで証拠の適用可能性を判断し、必要に応じて系譜として接続する仕組みは v0.1 以降で設計する。
...
- v0.0 の間は、実装の探索に合わせてこの文書を随時改定する。
