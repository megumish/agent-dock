...
この文書は、[VISION.md](../VISION.md) が定める用語に対応する英語表記を記録する、非規範の一時的な対訳メモである。
ADR を拘束せず、実装で識別子を確定する際に見直す。
実装のコード、識別子、コミットメッセージで用語を英語で書くときの出発点として参照する。
用語の意味はこの文書では定義せず、VISION の定義に従う。

対訳は、[DESIGN.md](../DESIGN.md) が識別子として既に使っている英語（運用モードの `shadow`、資格状態の `qualified` など）と揃えている。
選定に判断が入った語には、備考欄に理由を示す。

VISION 本文への併記は、VISION が定義するプロジェクト固有の概念の初出に限る。
フィードバックやリプレイのようなカタカナ語でも、固有の概念として定義する語には併記する。
一方、構成や鮮度のような一般的な語は、この表には載せるが本文には併記しない。
