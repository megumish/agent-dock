...
4. [現在のリリース文書](./docs/releases/0.1.0-alpha.10.md)

## 0.1.0-alpha.10

0.1.0-alpha.10 は、一つのタスクを登録済みの実行プロファイルへ依頼する仲介実行、ユーザーが直接起動した Claude Code と Codex CLI の対話セッションの観測、スコアカードを根拠とする助言を提供する。
...
0.1.0-alpha.10 は実行時の CLI 版とモデルの観測までを対象とし、実支出は取得しない。
現在の実装範囲と挙動は [0.1.0-alpha.10 のリリース文書](./docs/releases/0.1.0-alpha.10.md) に記録する。
