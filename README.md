...
文書は**VISION（最上位の規範）→ DESIGN（v0.0 の設計）→ ADR（v0.1 以降の決定と理由）→ 実装**の順に具体化する。
...
| [DESIGN.md](./DESIGN.md) | 最小実装 v0.0 の構成と不変条件を定め、v0.0 で実現しない VISION の要求を列挙する設計文書 |
| [DEPENDENCIES.md](./DEPENDENCIES.md) | 外部 crate の採用、更新、再評価に適用するプロジェクト共通の方針 |
| [docs/releases/](./docs/releases/README.md) | パッチ版ごとの実装範囲と運用上の挙動を記録するリリース文書 |
